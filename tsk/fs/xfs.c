    ...
    TSK_INUM_T inum, const xfs_dinode * dino_buf)
{
    ...
    //     tsk_getu32(fs->endian, ext2fs->fs->sb_icount));
    ...
    //     ext2fs->fs->sb_icount) + fs->first_inum;
    ...
    xfs_dinode * dino_buf)
{
    ...
    switch (dino_core_buf->di_format) {
        ...
        case 0x1:
            ...
            cnt = tsk_fs_read(fs, addr, (char *) di_dir2sf, sizeof(xfs_dir2_sf_t));
            if (cnt != sizeof(xfs_dir2_sf_t)) {
                if (cnt >= 0) {
                    tsk_error_reset();
                    tsk_error_set_errno(TSK_ERR_FS_READ);
                }
...
//                 tmpInt, (100 * tmpInt) /  
//                 tsk_getu32(fs->endian, sb->sb_icount));
...
//             if (inum_left == 0)
//                 inum_left = tsk_getu32(fs->endian, sb->sb_icount);
